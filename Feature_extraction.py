...
#This function returns the mode of each slide window

def gesture_estimation(signal, channel_name, fs, frame, step, plot=False):


    gesture_names = ['TASK']

    gesture_matrix = task_mode(signal, frame, step)
    

    print('EMG features were from channel {} extracted successfully'.format(channel_name))

    if plot:
        plot_features(signal, channel_name, fs, gesture_matrix, step)

    return gesture_matrix


def task_mode(signal, frame, step):
    ...
    for i in range(frame, signal.size, step):
        ...
        task.append(statistics.mode(x))
       

    gesture_matrix = np.column_stack((task))
    return gesture_matrix
